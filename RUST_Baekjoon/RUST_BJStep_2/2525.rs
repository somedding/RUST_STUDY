...
오븐에 넣으면 된다. 그러면 인공지능 오븐은 오븐구이가 끝나는 시간을 분 단위로 자동적으로 
계산한다. 
...
fn main() {
    ...
    let mut endtimes = String::new();
    
    io::stdin().read_line(&mut time_input).unwrap();
    io::stdin().read_line(&mut endtimes).unwrap();
    
    let numbers: Vec<&str> = time_input.split_whitespace().collect();

    let mut hour: i32 = numbers[0].parse::<i32>().unwrap();
    let mut min: i32 = numbers[1].parse::<i32>().unwrap();

    let resulttime: i32 = endtimes.trim().parse().unwrap();
    hour = hour + resulttime / 60;
    min = min + resulttime % 60;
